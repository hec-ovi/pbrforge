...
Each shipped street style also has additive `constructionSurfaces`: `pavingBody`, `joint`, `border` and `curb`, each a `{ kind, variant }` binding. Every construction finish is continuous, isotropic and has no structural marks. `street-paving-body/mid` covers 2 x 2 m at 1024 px with 6-11 mm aggregate, restrained 55 mm binder variation and sparse local wear; `street-joint/mid` covers 0.5 x 0.5 m at 512 px. Both alias all tiers and contain the three style IDs. Border and curb reuse their continuous street finishes. Geometry owns slab boundaries, physical joint regions, bevels and all fit. Apply these materials at their published metre scale, without adding texture seams to the slab body. [Construction recipes](batch/cyberpunk/street-construction.json) regenerate the six finishes.
