...
Each shipped street style also has additive `constructionSurfaces`: `pavingBody`, `joint`, `border`, `curb`, `road` and `gutter`, each a `{ kind, variant }` binding. Road and gutter are optional for older binding documents. Every construction finish is continuous, isotropic and has no structural marks. Geometry owns slabs, joints, curb faces, drainage lips and all fit; a gutter is a distinct road-side region, even when it shares a finish with a border. Apply the published metre scale independently of cell pitch. [Street image recipes](batch/cyberpunk/street-image-finishes.json) create the selected 1 x 1 m, 1024 px road/precast/graphite finishes with all-tier aliases. Each family has its own dry roughness band and canonical tone variant: maintained neutral, salvaged darker warm-grey paving with lighter asphalt, industrial cool-grey paving with darker asphalt. These are candidate finishes, without fitted-city visual acceptance.
