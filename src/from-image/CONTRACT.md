...
- `path`: PNG (or JPEG converted by the caller to PNG). Must cover `resolution`, same aspect, fully opaque. Downsample only.
- `alignment`: `exact` (asymmetric faces: AC, door, poster) needs `aspect`. `tile` (bricks, concrete, walls) needs `tiling.worldSize`. Tile does not check wrap.
...
- Height comes from albedo luminance. Normal and AO from that height. Roughness from the finish band. Metallic is the factor, flat.
